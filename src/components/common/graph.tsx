export function getCanvas(item: any) {
...
function px2cm(px: any, dpi: any) {
  return px * (2.54 / dpi);
}

function cm2px(cm: any, dpi: any) {
  return cm * (dpi / 2.54);
}

function hourDiff(a: any, b: any) {
  return Math.abs(a - b) / 36e5;
}

function getHourString(n: any) {
  var h: any = Math.floor(n);
  var m: any = Math.round((n - h) * 60);
...
class Config {
  ...
  constructor(config: any) {
    this._config = setConfig(config) as any;
    this.scrubData();
  }
  ...
  scrubData = () => {
    ...
    if (data.length > 0) {
      ...
      for (var j = remove.length - 1; j >= 0; j--) data.splice(j, 1);
...
export class eLog {
  ...
  lineHeightMult: any;
  minFontSize: any;
  headerFontPercent: any;
  statusFontPercent: any;
  hourFontPercent: any;
  fontFamily: any;
  bgColor: any;
  lineColor: any;
  ...
  hourTextColor: any;
  violationColor: any;
  violationLineColor: any;
  headerRatio: any;
  statusRatio: any;
  totalRatio: any;
  tenHourConsecutive: any;
  drivingConsecutive: any;
  sleeperConsecutive: any;
  sevenHourProvision: any;
  drivingBreak: any;
  elevenHourDriving: any;
  fourteenHourLimit: any;
  constructor(item: any, userConfig: any) {
    const initCanvas: any = getCanvas(item);
    const context = getCtx(initCanvas);
    const canvas: any = context && context.canvas;
    const height = canvas && canvas.height;
    const width = canvas && canvas.width;

    this.config = new Config(userConfig) as any;
    this.ctx = context;
    this.canvas = canvas;
    this.width = width;
    this.height = height;
  ...
  addData = (addStatus: any, addDate: any) => {
    this.data.push({
      status: addStatus,
      d: addDate,
    });
  ...
  printFillText = (txt: any, fs: any, x: any, y: any, w = 0) => {
    const ctx = this.ctx;
  ...
  drawSingleLine = (x: any, y: any, x2: any, y2: any, c: any, w: any) => {
    const ctx = this.ctx;
  ...
  checkViolations = (
    ...
    if (prevStatus == 1 || prevStatus == 2) {
      ...
      if (
        this.tenHourConsecutive >= 10 ||
        (this.tenHourConsecutive >= 2 &&
          this.tenHourConsecutive < 7 &&
          this.sleeperConsecutive >= 7 &&
          this.tenHourConsecutive + this.sleeperConsecutive >= 10)
      ) {
        // reset limits
    ...
    if (
      this.elevenHourDriving > 11 || // 11-Hour Driving Limit
      this.drivingConsecutive > 8 || // 30-Minute Driving Break
      (this.fourteenHourLimit > 14 && curStatus == 3)
    ) {
      // 14-Hour Limit
  ...
  drawElog = () => {
    ...
    const hourBlockWidth = Math.floor(
      (elogWidth - statusBlockWidth - totalBlockWidth) / 24
    );
    ...
    for (var i = 0; i < 24; i++) {
      ...
      this.printFillText(
        hourText,
        headerFontSize,
        hourBlockWidth * i + statusBlockWidth,
        headerBlockHeight
      );
    }
    ...
    for (var i = 1; i < 5; i++) {
      ...
      this.printFillText(
        ...
        statusFontSize,
        Math.floor(statusBlockWidth * 0.5),
        Math.floor(rowY - statusBlockHeight * 0.5),
        statusBlockWidth
      );
      ...
      for (var j = 1; j < 25; j++) {
        ...
        var xCoor1 = hourX - Math.floor(hourBlockWidth * 0.875);
        var xCoor2 = hourX - Math.floor(hourBlockWidth * 0.75);
        var xCoor3 = hourX - Math.floor(hourBlockWidth * 0.625);
        var xCoor4 = hourX - Math.floor(hourBlockWidth * 0.5);
        var xCoor5 = hourX - Math.floor(hourBlockWidth * 0.375);
        var xCoor6 = hourX - Math.floor(hourBlockWidth * 0.25);
        var xCoor7 = hourX - Math.floor(hourBlockWidth * 0.125);
        ...
        ctx.moveTo(
          xCoor1,
          rowY -
            Math.floor(statusBlockHeight * 0.75 * 0.5) -
            statusBlockHeight * 0.25
        );
        ctx.lineTo(
          xCoor1,
          rowY -
            Math.floor(statusBlockHeight * 0.25 * 0.5) -
            statusBlockHeight * 0.25
        );
        ctx.stroke();
        ...
        // half hour line
        ctx.beginPath();
        ctx.moveTo(xCoor2, rowY - Math.floor(statusBlockHeight * 0.68));
        ctx.lineTo(xCoor2, rowY - Math.floor(statusBlockHeight * 0.35));
        ctx.stroke();
        ...
        // ------
        ctx.beginPath();
        ctx.moveTo(xCoor4, rowY - Math.floor(statusBlockHeight * 0.75));
        ctx.lineTo(xCoor4, rowY - Math.floor(statusBlockHeight * 0.25));
        ctx.stroke();
        ...
        //-----2
        ctx.beginPath();
        ctx.moveTo(xCoor6, rowY - Math.floor(statusBlockHeight * 0.68));
        ctx.lineTo(xCoor6, rowY - Math.floor(statusBlockHeight * 0.35));
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(
          xCoor7,
    ...
    this.drawTotals(
      Math.ceil(headerBlockWidth + (elogWidth - headerBlockWidth) * 0.5),
      elogWidth - headerBlockWidth,
      statusBlockHeight,
      hourFontSize
    );
  ...
  drawLines = (hourBlockWidth: any) => {
    ...
    if (data.length > 0) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        var toMin: any = (toDate.getMinutes() / 60).toFixed(2);
        ...
        if (prevData) {
          ...
          var fromMin: any = (fromDate.getMinutes() / 60).toFixed(2);
          ...
          this.drawSingleLine(
            this.hours[fromHr] + Math.floor(hourBlockWidth * fromMin),
            statuses[prevData.status].y,
            this.hours[toHr] + Math.floor(hourBlockWidth * toMin),
            statuses[prevData.status].y,
            lineC,
            this.hourLineWidth
          );
          ...
          this.drawSingleLine(
            this.hours[toHr] + Math.floor(hourBlockWidth * toMin),
            statuses[prevData.status].y + this.hourLineWidth * 0.5 * (dir * -1),
            this.hours[toHr] + Math.floor(hourBlockWidth * toMin),
            statuses[data[i].status].y + this.hourLineWidth * 0.5 * dir,
            this.vertLineColor,
            this.vertLineWidth
          );
        } else {
          ...
          this.drawSingleLine(
            this.hours["0"],
            this.statuses[this.startStatus].y,
            this.hours[toHr] + Math.floor(hourBlockWidth * toMin),
            statuses[this.startStatus].y,
            lineC,
            this.hourLineWidth
          );
      ...
      var finalDate: any = new Date(prevData.d);
      var finalHr: any = finalDate.getHours();
      var finalMin: any = (finalDate.getMinutes() / 60).toFixed(2);
      ...
      if (statuses[prevData.status].violation) lineC = this.violationLineColor;

      this.drawSingleLine(
        this.hours[finalHr] + Math.floor(hourBlockWidth * finalMin),
        statuses[prevData.status].y,
        this.hours["24"],
        statuses[prevData.status].y,
        lineC,
  ...
  drawTotals = (x: any, wd: any, ht: any, fs: any) => {
    const ctx = this.ctx;
    const data = this.data;
    const statuses = this.statuses;
    ...
    if (data.length > 0) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (prevData) {
          ...
          statuses[this.startStatus].h =
            statuses[this.startStatus].h + startDiff;
        }
  ...
  printData = (x: any, startY: any, fs: any) => {
    const ctx = this.ctx;
    const data = this.data;
    const statuses = this.statuses;
    ...
    if (data.length > 0) {
      ...
      y = y + lineHeight * 0.5;
      this.printFillText(
        "1. " + statuses[this.startStatus].label.replace("\n", " "),
        fs,
        x,
        y
      );
